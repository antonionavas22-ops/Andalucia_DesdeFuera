...
#localidades
write_csv(resultado, "resultado_localidades_Calvert.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Calvert.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Calvert.csv") 
...
ggplot(top25, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Menciones en Calvert (1903)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")
#dédimotercer paso o epílogo: exportación a imagen
ggsave("grafica_Williams.png", width = 12, height = 8) 