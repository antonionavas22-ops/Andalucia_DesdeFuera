...
#localidades
write_csv(resultado, "resultado_localidades_Elliott.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Elliott.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Elliott.csv") 
...
ggplot(top25, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Menciones en Elliott (1908)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")
#dédimotercer paso o epílogo: exportación a imagen
ggsave("grafica_Elliott1908.png", width = 12, height = 8) 