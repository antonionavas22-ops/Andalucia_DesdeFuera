...
relato <- readLines("1908_Calvert_southern_spain.txt" , encoding = "UTF-8")
...
#localidades
write_csv(resultado, "resultado_localidades_Calvert_southern_spain.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Calvert_southern_spain.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Calvert_southern_spain.csv") 
...
ggplot(top20, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Frecuencia en southern spain de Calvert (1908)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")

#por último se exporta la imagen ajustada
ggsave("1908_patrimonio__Calvert_southern_spain.png", width = 12, height = 8) 
...
barplot(totales,
        las = 2,
        col = colores,
        main = "Análisis de sentimientos - calvert 1908  Southern Spain",
...
write.csv(totales, "1908_sentimientos_Calvert_Southern_spain.csv")
