...
relato <- readLines("1900_Bates_spanish_highways_byways.txt", encoding = "UTF-8")
...
ggplot(top20, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Frecuencia en Spanish highways and byways de Bates (1900)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")

#por último se exporta la imagen ajustada
ggsave("1900_grafica_Bates_spanish_highways_byways.png", width = 12, height = 8) 
...
barplot(totales,
        las = 2,
        col = colores,
        main = "Análisis de sentimientos - Bates 1900 Spanish Highways Byways",
...
write.csv(totales, "sentimientos_Bates.csv")
