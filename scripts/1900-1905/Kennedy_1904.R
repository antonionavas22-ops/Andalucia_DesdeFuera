...
relato <- readLines("1904_Kennedy_tramp_spain.txt" , encoding = "UTF-8")
...
#localidades
write_csv(resultado, "resultado_localidades_Kennedy_tramp_spain.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Kennedy_tramp_spain.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Kennedy_tramp_spain.csv") 
...
ggplot(top20, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Frecuencia en A tramp in Spain de Kennedy (1904)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")

#por último se exporta la imagen ajustada
ggsave("1904_patrimonio_Kennedy_tramp_Spain.png", width = 12, height = 8) 
...
barplot(totales,
        las = 2,
        col = colores,
        main = "Análisis de sentimientos - Kennedy 1904 Tramp in Spain",
...
write.csv(totales, "1904_sentimientos_Kennedy_tramp_Spain.csv")
