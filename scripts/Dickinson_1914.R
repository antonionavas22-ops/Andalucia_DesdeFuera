...
relato <- readLines("1914_dickinson_through_spain.txt" , encoding = "UTF-8")
...
#localidades
write_csv(resultado, "resultado_localidades_Dickinson_through_spain.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Dickinson_through_spain.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Dickinson_through_spain.csv")
...
ggplot(top20, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Frecuencia en through spain de Dickinson (1914)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")

#por último se exporta la imagen ajustada
ggsave("1914_patrimonio__Dickinson_through_spain.png", width = 12, height = 8) 
...
barplot(totales,
        las = 2,
        col = colores,
        main = "Análisis sentimientos -Dickinson 1914 Through spain",
...
write.csv(totales, "1914_sentimientos_Dickinson_through_spain.csv")
