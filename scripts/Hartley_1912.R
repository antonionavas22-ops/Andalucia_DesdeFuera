...
relato <- readLines("1912_Hartley_things_seen.txt" , encoding = "UTF-8")
...
#localidades
write_csv(resultado, "resultado_localidades_Hartely_things_seen.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Hartely_things_seen.csv") 
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Hartely_things_seen.csv")  
...
ggplot(top20, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Frecuencia en things seen de Hartley (1912)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")

#por último se exporta la imagen ajustada
ggsave("1912_patrimonio__hartley_things_seen.png", width = 12, height = 8) 
...
barplot(totales,
        las = 2,
        col = colores,
        main = "Análisis de sentimientos - hartley 1912 things seen",
...
write.csv(totales, "1912_sentimientos_hartley_things_seen.csv")
