...
relato <- readLines("1909_Shaw_spain_today.txt" , encoding = "UTF-8")
...
#localidades
write_csv(resultado, "resultado_localidades_Shaw_spain_today.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Shaw_spain_today.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Shaw_spain_today.csv") 
...
ggplot(top20, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Frecuencia en Spain today de Shaw (1909)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")

#por último se exporta la imagen ajustada
ggsave("1909_patrimonio__Shaw_spain_today.png", width = 12, height = 8) 
...
barplot(totales,
        las = 2,
        col = colores,
        main = "Análisis de sentimientos - Shaw 1909 Spain today  ",
...
write.csv(totales, "1909_sentimientos_Shaw_Spain_today.csv")
