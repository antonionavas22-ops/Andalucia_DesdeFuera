...
#localidades
write_csv(resultado, "resultado_localidades_Buck_Chapman.csv") 
#patrimonio material
write_csv(resultado_material, "resultado_material_Buck_Chapman.csv")
#patrimonio inmaterial
write_csv(resultado_inmaterial, "resultado_inmaterial_Buck_Chapman.csv") 
...
ggplot(top25, aes(x = reorder(principal, menciones), y = menciones, fill = tipo)) +
  ...
  labs(title = "Menciones en Buck y Chapman (1910)",
       x = NULL, y = "Menciones") +
  theme_minimal() +
  theme(legend.position = "none")
#dédimotercer paso o epílogo: exportación a imagen
ggsave("grafica_Buck_Chapman.png", width = 12, height = 8) 